...
export function AdminPagesContainer({
  currentPage,
  children
}: {
  currentPage: string
        ...
        <Container fluid>
          ...
          >
            <Icon
              ...
              size={1}
              className={
                "text-white my-nav-icon " + (showDrawer ? "open" : "close")
              }
            />
          </Button>
...
function ListOfLinks(P: {
  ...
  return (
    <ListGroup className={P.className ?? ""} style={P.style}>
      {links.map((i) => (
        <Link
          href={`/admin/${i.name}`}
          key={i.name}
          className="text-decoration-none rounded-4 border mb-1"
        >
          <ListGroup.Item
            ...
            onClick={P.hideMenu}
          >
            <Icon path={i.icon ?? mdiHomeAssistant} size={1} className="ms-2" />
            {i.text}
          </ListGroup.Item>
        </Link>
      ))}
      <Link
        className="text-decoration-none rounded-4 mb-1"
        href="/admin"
        onClick={() => {
      ...
      >
        <ListGroup.Item className="bg-danger text-white">
          <Icon path={mdiLocationExit} size={1} className="ms-2" />
          خروج
        </ListGroup.Item>
      </Link>
    </ListGroup>
