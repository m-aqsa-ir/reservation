...
function SectionIndicator(p: {
  state: "passed" | "current" | "remained"
  section: Section
}) {
  ...
  return (
    <div className="d-flex flex-column align-items-center me-2 me-lg-4">
      <Icon path={p.section.icon} size={1} className={`${colorForState}`} />
      <span className={`${colorForState} text-center SectionIndicator-Span`}>
        {p.section.name}
      </span>
    </div>
  )
