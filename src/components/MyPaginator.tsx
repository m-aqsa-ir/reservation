...
export function MyPaginator({ page, pageCount, totalCount, pageName }: PaginatorState & { pageName: string },) {
  const maxPageCount = Math.ceil(totalCount / pageCount)

  const pages = [page - 2, page - 1, page, page + 1, page + 2].filter(i => i >= 1 && i <= maxPageCount)
  ...
  return <Pagination className="justify-content-center">
    {showLeftDoubleArrow ? <Pagination.First href={`${pageName}?page=${1}`} /> : <></>}
    ...
    {showRightDoubleArrow ? <Pagination.Last href={`${pageName}?page=${maxPageCount}`} /> : <></>}
  </Pagination>
}