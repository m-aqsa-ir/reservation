...
export default function Home(props: IndexPageProps) {
  ...
  const packages = () =>
    chosenDay == null
      ? []
      : chosenDay.services.filter((i) => i.type == "package")
  const services = () =>
    chosenDay == null
      ? []
  ...
  const [servicesOrPackage, setServicesOrPackage] = useState<
    "package" | "services"
  >("package")
  ...
  function calcPrice() {
    ...
    const wholePrice =
      chosenServices.reduce(
        (sum, i) => sum + (chosenDay.isVip ? i.priceVip ?? 0 : i.priceNormal),
        0
      ) * volume
  ...
  async function handleChangeVol(e: ChangeEvent<HTMLSelectElement>) {
    ...
    if (value == null) {
      ...
      handleFetchDay(
        props.volumeList.find((v) => v.id == Number(eValue))!.volume
      )
      setChosenDay(null)
    }
  }
    ...
    <PageContainer>
      ...
      >
        ...
        {props.volumeList.map((i) => (
          <option key={i.id} value={i.id}>
            {enDigit2Per(i.volume)} نفر{" "}
            {i.discountPercent == 0 ? (
              ""
            ) : (
              <>
                &nbsp;&nbsp;-&nbsp;&nbsp; {enDigit2Per(i.discountPercent)}%
                تخفیف
              </>
            )}
          </option>
            ...
            {servicesOrPackage == "package"
              ? packages().map((pac) => (
                  <PackageComponent
                    ...
                    onReserve={() => {
                      const anotherPac = chosenServices.find(
                        (i) => i.type == "package"
                      )
                      if (anotherPac == undefined) {
                        ...
                        setChosenServices([
                          ...chosenServices.filter((i) => i.type != "package"),
                          pac
                        ])
                      } else {
                        setChosenServices(
                          chosenServices.filter((i) => i.id != pac.id)
                        )
                      }
                    }}
              ...
              : services().map((s) => (
                  <PackageComponent
                    ...
                    onReserve={() => {
                      if (
                        chosenServices.find((j) => j.id == s.id) == undefined
                      ) {
                        setChosenServices([...chosenServices, s])
                      } else {
                        setChosenServices(
                          chosenServices.filter((j) => j.id != s.id)
                        )
                      }
                    }}
                  />
      ...
      <Row className={"align-items-baseline mt-3 rounded-4 my-index-end-part "}>
        <Col lg="6" className="mb-lg-0 mb-3">
          ...
          >
            با کلیک روی تایید و ادامه با قوانین و مقررات سایت موافقت کرده‌اید.
          </p>
        </Col>
      ...
      <Modal
        show={changeVolumeMode != null}
        onHide={() => setChangeVolumeMode(null)}
      >
        <Modal.Body>
          با انتخاب این روز، ظرفیت شما به
          <span className="fw-bold">
            &nbsp; {enDigit2Per(changeVolumeMode?.availableWith ?? 0)} &nbsp;{" "}
          </span>
          نفر تغییر خواهد کرد. آیا مطمئنید؟
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="success"
            onClick={(e) => {
              ...
              const chosenVol =
                props.volumeList.find(
                  (i) => i.volume == changeVolumeMode.availableWith
                ) ?? null
              if (chosenVol != null) {
...
function PackageComponent(p: {
  ...
  return (
    <Row className="border rounded-4 m-2 p-2">
      <Col md="9">
      ...
      <Col md="3">
        <div className="d-flex flex-column justify-content-center align-items-center">
          <p>
            {enNumberTo3DigPer(
              p.vipDay ? p.pac.priceVip ?? 0 : p.pac.priceNormal
            )}{" "}
            تومان
          </p>
          <Button
            variant={p.reserved ? "success" : "primary"}
            onClick={p.onReserve}
            className="w-100"
          >
