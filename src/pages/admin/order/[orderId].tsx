...
export default function OrderDetailsPage(P: OrderDetailsPageProps) {
  ...
  async function handleEdit(
    newOrder: OrderEditPayload & { services: Service[]; day: OurDay }
  ) {
    ...
    if (res.ok) {
      const {
        calculatedAmount,
        serviceIds
      }: { calculatedAmount: number; serviceIds: number[] } = await res.json()
      ...
      setOrder({
        ...
        OrderService: services
          .filter((i) => serviceIds.includes(i.id))
          .map((i, index) => {
            return {
              id: index,
              isVip: newDay.isVip,
      ...
      <Row className="bg-white border rounded-4 p-2">
        <Col md="9">
          <Row>
            <Col md="12">
              <h1 className="fs-3">
                مشخصات سفارش به شناسه {enDigit2Per(order.id)}
              </h1>
              <hr />
            </Col>
            ...
            <Col md="6">
              <InfoItem
                name="زمان ثبت"
                value={time2Str(order.timeRegistered, "", true)}
              />
            </Col>
            ...
            {order.cancelRequest && (
              <Col md="6" className="tw-bg-red-300 p-2 rounded">
        ...
        <Col md="3" className="mt-3 mt-md-3">
          ...
          <AdminTable columnNames={["نام", "نوع", "هزینه"]} notAddBottomMargin>
            <tbody>
...
function ActionIconButton(P: {
  ...
  return (
    <Button
      className={"mb-1 text-end " + (P.className ?? "")}
      onClick={P.onClick}
      variant={P.variant}
      href={P.href}
      target={P.target}
...
function AddTransactionModal(P: {
  show: boolean
  onHide: Function
  addPayState: AddTransaction | null
  ...
  return (
    <Modal show={addPayState != null} onHide={() => setAddPayState(null)}>
      {addPayState && (
        <Form
          onSubmit={(e) => {
            e.preventDefault()
            ...
            <Row className="align-items-center">
              ...
              <Col md="12" className="mt-2">
                <Form.Control
                  ...
                  onChange={(e) =>
                    setAddPayState((x) => ({ ...x!, desc: e.target.value }))
                  }
                  placeholder="توضیح"
                />
              </Col>
...
function EditOrderModal(P: {
  ...
  const [availableDays, setAvailableDays] = useState<
    (OurDay & { reserved: number; remained: number })[]
  >([])
  ...
  useEffect(() => {
    ;(async function () {
      if (P.show) {
        ...
        const res = await fetchPost("/api/admin/availables", {
          currentDayId: P.order.dayId
        })
        ...
        <Form
          onSubmit={(e) => {
            ...
            P.onEnd({
              ...order,
              volume: nVol,
              services: availableServices.filter((i) =>
                order.serviceIds.includes(i.id)
              ),
              day:
                order.dayId == P.order.dayId
                  ? P.currentDay
                  : availableDays.find((i) => order.dayId == i.id)!
            })
          }}
        >
          <Modal.Header>ویرایش روز</Modal.Header>

          <Modal.Body>
            <Form.Label>نام گروه</Form.Label>
            <Form.Control
              className="w-100 mb-2"
              value={order.groupName}
              required
              onChange={(e) =>
                setOrder({ ...order, groupName: e.target.value })
              }
            />
            ...
            <Accordion>
                  ...
                  >
                    ...
                    <ListGroup.Item className="d-flex justify-content-between p-2">
                      ...
                      <Form.Check
                        ...
                        onChange={() =>
                          setOrder({ ...order, dayId: P.currentDay.id })
                        }
                      />
                    </ListGroup.Item>
              ...
              <Accordion.Item eventKey="2">
                ...
                <Accordion.Body className="p-0">
                  <ListGroup>
                    {availableServices
                      .filter((i) => i.type == serviceTypeEnum.package)
                      .map((i) => (
                        <ListGroup.Item
                          key={i.id}
                            ...
                            onChange={(e) =>
                              setOrder(
                                e.target.checked
                                  ? {
                                      ...order,
                                      serviceIds: [...order.serviceIds, i.id]
                                    }
                                  : {
                                      ...order,
                                      serviceIds: order.serviceIds.filter(
                                        (j) => j != i.id
                                      )
                                    }
                              )
              ...
              <Accordion.Item eventKey="3">
                ...
                <Accordion.Body className="p-0">
                  <ListGroup>
                    {availableServices
                      .filter((i) => i.type == serviceTypeEnum.service)
                      .map((i) => (
                        <ListGroup.Item
                          key={i.id}
                          className="d-flex p-2 justify-content-between"
            ...
            {P.order.Discount.length != 0 && (
              <div className="d-flex align-items-center justify-content-between mt-2">
                حذف تخفیف ها
                <Form.Check
                  checked={order.deleteDiscounts}
                  onChange={(e) =>
                    setOrder({ ...order, deleteDiscounts: e.target.checked })
                  }
                />
              </div>
            )}
    ...
    async callbackSuccess(prisma) {
      ...
      return {
        props: {
          order: {
            ...order,
            paidAmount: order.Transaction.reduce(
              (sum, i) => sum + i.valuePaid,
              0
            ),
            cancelRequest:
              order.OrderCancel.length == 0 ? null : order.OrderCancel[0].reason
          }
