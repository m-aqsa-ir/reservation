...
export default function AdminVolumeList(props: ListsPageProp) {
  return (
    <AdminPagesContainer currentPage="lists">
      <Head>
      ...
      >
        ...
        >
              ...
              <Row>
                <Col md="6" className="mb-2">
                  <PerNumberInput
                    ...
                    onChange={(e) =>
                      setAddRowState((r) => ({ ...r!, volume: e.target.value }))
                    }
                  />
                </Col>
                <Col md="6">
          ...
          <Modal.Footer>
            ...
            />
            <IconButton iconPath={mdiCheck} type="submit" variant="success" />
          </Modal.Footer>
        </Form>
      </ModalFonted>
    </>
...
function GroupsListPart(props: { groups: GroupType[] } & TablePageBaseProps) {
  ...
  async function handleEdit() {
    ...
    if (res.ok) {
      setGroups((gs) =>
        gs.map((g) => (g.id == editMode.id ? { ...g, ...editMode } : g))
      )
      setEditMode(null)
    } else {
      resHandleNotAuth(res, dispatch, router)
    ...
    <>
      <div className="d-flex justify-content-between mb-3 align-items-base mt-3 mt-lg-0">
        ...
        <Button
          ...
          onClick={() =>
            setAddMode((a) =>
              a ? null : { name: "", iconPath: listAvailableIcons[0] }
            )
          }
        >
          اضافه کردن <Icon path={mdiPlus} size={1} />
          ...
          {groups.map((i) => (
            <Fragment key={i.id}>
              <tr key={i.id}>
                ...
                <td>
                  <Icon
                    path={i.iconPath == "" ? mdiBorderNoneVariant : i.iconPath}
                    size={1}
                  />{" "}
                </td>
                {/* ACTION */}
              ...
              {listAvailableIcons.map((i) => (
                <div key={i} className="d-flex align-items-center flex-column">
                  ...
                  <Form.Check
                    ...
                    value={i}
                    onChange={(e) =>
                      setEditMode({ ...editMode!, iconPath: e.target.value })
                    }
                  />
                </div>
              ))}
        ...
        >
          <Modal.Body>
            <Form.Control
              ...
              placeholder="نام"
              onChange={(e) =>
                setAddMode({ ...addMode!, name: e.target.value })
              }
            />
            <p className="text-center mt-2">انتخاب آیکون</p>
            <div className="d-flex justify-content-around mt-3 flex-wrap">
              {listAvailableIcons.map((i) => (
                <div key={i} className="d-flex align-items-center flex-column">
                  ...
                  <Form.Check
                    ...
                    value={i}
                    onChange={(e) =>
                      setAddMode({ ...addMode!, iconPath: e.target.value })
                    }
                  />
                </div>
              ))}
    ...
    callbackSuccess: async (prisma) => {
      ...
      return {
        props: {
          volumeList: {
            vs,
            columnNames: ["ظرفیت", "درصد تخفیف", "عملیات"]
          },
          groupList: {
            groups,
            columnNames: [
