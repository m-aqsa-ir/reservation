...
export default function AdminTransactionPage(props: AdminTransactionProps) {
  ...
  return (
    <AdminPagesContainer currentPage="transaction">
      <Head>
        ...
        <tbody>
          {transactions.map((i) => (
            <tr key={i.id}>
              ...
              <td style={{ wordWrap: "break-word", fontSize: "0.7rem" }}>
                {i.payId}
              </td>
              <td>{i.payPortal != "cash" ? i.payPortal : i.desc}</td>
              <td className="text-nowrap">{i.payDate}</td>
              {!props.forOrderDetail ? (
